...
# Taking input from user for exact date
dd = int(input('Enter a date :'))
mm = int(input('Enter a month :'))
yy = int(input('Enter a year :'))
# Using if else to check if date entered by user is valid or invalid
if dd <= 0 or mm <= 0 or yy <= 0:
    print('Enter a valid date')
else:
    # Putting the inputs under a variable and shifting the variable to next date
    date = datetime(yy, mm, dd)
    next_date = date + timedelta(days=1)
    # Printing the required result
    print('Next date will be :', next_date)
